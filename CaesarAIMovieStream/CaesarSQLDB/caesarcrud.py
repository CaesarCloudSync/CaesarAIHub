...
from CaesarSQLDB.caesarsql import CaesarSQLAsync
class CaesarCRUD:
...
class CaesarCRUDAsync:
    async def __aenter__(self) -> None:       
        return self

    async def create_table(self,primary_key:str,fields:tuple,types :tuple,table: str):
        if type(fields) == tuple:
            fieldlist = [f"{field} {typestr}"for field,typestr in zip(fields,types)]
            fieldstr = ', '.join(fieldlist)
            try:
    ...
    async def post_data(self,fields:tuple,values:tuple,table:str):
    ...
    async def get_data(self,fields:tuple,table:str,condition=None,getamount:int=1000):
    ...
    def hex_to_base64(self,hex_file:bytes): # x0 unicode-like hex
        return  base64.b64encode(bytes.fromhex(hex_file.hex())).decode()
    async def get_large_data(self,fields:tuple,table:str,condition=None):
        ...
        if condition:
            #print(f"""SELECT {fieldstr} FROM {table} WHERE {condition};""")
            async with CaesarSQLAsync() as caesarsqlcm:
                result = caesarsqlcm.run_command_generator(f"""SELECT {fieldstr} FROM {table} WHERE {condition};""")
            return result
        else:
            async with CaesarSQLAsync() as caesarsqlcm:
                result = caesarsqlcm.run_command_generator(f"""SELECT {fieldstr} FROM {table};""")
            return result

        
    async def update_data(self,fieldstoupdate:tuple,values:tuple,table=str,condition=str):
        if len(fieldstoupdate) > 1:
            updatelist = []
            for field,value in zip(fieldstoupdate,values):
                if type(value) != str:
            ...
            updatestr = ', '.join(updatelist)
            async with CaesarSQLAsync() as caesarsqlcm:
                result = caesarsqlcm.run_command(f"UPDATE {table} SET {updatestr} WHERE {condition} returning {fieldstoupdate[0]};",caesarsqlcm.fetch)
            if len(result) == 0:
                return True
            else:
                return False
            ...
            if len(result) == 0:
                ...
                return False
    # TODO Cgeck This works
    async def update_blob(self,fieldstoupdate:str,value:str,table=str,condition=str):
        updatestr = "UPDATE %s SET %s = x'%s' WHERE %s returning %s;" % (table,fieldstoupdate,self.base64_to_hex(value),condition,fieldstoupdate[0])
        async with CaesarSQLAsync() as caesarsqlcm:
            result = caesarsqlcm.run_command(updatestr,caesarsqlcm.fetch)
        if len(result) == 0:
            return True
        else:
            return False
    async def delete_data(self,table:str,condition:str):
        field_name = condition.split("=")[0].strip()
        async with CaesarSQLAsync() as caesarsqlcm:
            result = caesarsqlcm.run_command(f"DELETE FROM {table} WHERE {condition} returning {field_name};",caesarsqlcm.fetch)
        if len(result) == 0:
            return True
        else:
            return False
        
    async def check_exists(self,fields:tuple,table:str,condition=None):
        if len(fields) != 1:
            fieldlist = [f"{field}" for field in fields]
            fieldstr = ', '.join(fieldlist) 
        else:
