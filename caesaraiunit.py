...
class CaesarAIUnittest(unittest.TestCase):
    def test_get_streaming_link(self):
        response = requests.get(f"{base_url}/caesaraimoviestream/8081/api/v1/get_single_episodes",params={"title":title,"season":season,"episode":episode})
        print(response.json())
        ...
        print(magnet_link)
        response = requests.post(f"{base_url}/caesaraimoviestream/8081/api/v1/torrent_magnet",json={"magnet_link":magnet_link})
        ...
        while status_and_progress.status != "downloaded":
            response = requests.get(f"{base_url}/caesaraimoviestream/8081/api/v1/check_torrent",params={"_id":status_and_progress.id})
            status_and_progress = StatusAndProgressResponse.model_validate(response.json())
            self.assertNotEqual(status_and_progress.id,None)
        response = requests.get(f"{base_url}/caesaraimoviestream/8081/api/v1/get_streaming_link",params={"_id":status_and_progress.id})
